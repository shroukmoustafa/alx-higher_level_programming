...
from duplicate_comparison2 import check_duplicates
...
# remove rows where all values are None (empty rows)
    ...
    content1['amount'] = content1['amount'].abs()
    ...
    #cleaning the currency defference names
    currency_map = {
    'LE': 'EGP',
    'L.E': 'EGP',
    'EGYPTIAN POUND': 'EGP'
    ...
    content1['currency'] = content1['currency'].replace(currency_map)
    content2['currency'] = content2['currency'].replace(currency_map)
#keep only the relevant columns for comparison
    df1 = content1[['date', 'amount', 'currency', 'name']]
    df2 = content2[['date', 'amount', 'currency', 'name']]
